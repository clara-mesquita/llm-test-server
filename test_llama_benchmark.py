#!/usr/bin/env python3
"""Short test: run llama-benchmark.generate() against a local mock server
(no GPU/Docker needed) to verify request format, SSE parsing, timing, and that
the output schema matches vllm-benchmark.py's fields."""
...
lb = load_module("llama_benchmark", "llama-benchmark.py")
...
def main():
    ...
    lb.PORT = srv.server_address[1]
    threading.Thread(target=srv.serve_forever, daemon=True).start()
    try:
        r = lb.generate("test/x", "What is 2+2?")
        ...
        # same tiers/prompts as vllm-benchmark.py -> same aggregate schema
        assert lb.PROMPTS == vb.PROMPTS, (lb.PROMPTS, vb.PROMPTS)
        assert set(lb.MODELS) == set(vb.MODELS), (set(lb.MODELS), set(vb.MODELS))
        for tier, fams in lb.MODELS.items():
            assert set(fams) == set(vb.MODELS[tier]), (tier, set(fams), set(vb.MODELS[tier]))

        print("OK: generate() parsed the stream and built a vllm-compatible run")
